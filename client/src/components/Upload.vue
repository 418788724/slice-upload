    ...
    <div class="content-wrapper">
      ...
      <div class="protocol-select">
        <div
          :class="
            protocol == 'http'
              ? 'protocol-item-http protocol-item'
              : 'protocol-item'
          "
        ...
        <div
          :class="
            protocol == 'websocket'
              ? 'protocol-item-ws protocol-item'
              : 'protocol-item'
          "
...
export default {
  ...
  methods: {
    ...
    async getFile() {
      ...
      await this.calFileBloomHash(this.file);
    },
    // 点击开始上传
    async startUpload() {
      const that = this;
    ...
    // 只计算头尾分片，中间分片取两字节生成的hash
    calFileBloomHash(file) {
      ...
      console.log(this.chunkSize, this.showChunkSize)
      return new Promise((resolve, reject) => {
        ...
        const that = this;
        this.updateTips('sucess', '正在读取文件计算hash中。。。');
        let bloomChunks = [file.slice(0, this.chunkSize)];
        let currentChunk = this.chunkSize;
        while (currentChunk < fileSize) {
          // 加最后一块
          if (currentChunk + this.chunkSize >= fileSize) {
            bloomChunks.push(file.slice(currentChunk, currentChunk + this.chunkSize))
          } else {
            // 中间分片，取前中后两个字节
            const mid = currentChunk + this.chunkSize / 2;
            const end = currentChunk + this.chunkSize;
            bloomChunks.push(file.slice(currentChunk. currentChunk + 2))
        ...
        fileReader.readAsArrayBuffer(new Blob(bloomChunks))
        
        console.log(spark, fileReader, that, bloomChunks, new Blob(bloomChunks), currentChunk / 1024)
        console.log('耗时1：', new Date() - timeStart);
        fileReader.onload = function(e) {
          ...
          console.log('耗时2：', new Date() - timeStart);
      ...
      return new Promise((resolve, reject) => {
        ...
        fileReader.onload = function (e) {
          ...
          if (currentChunk < chunks) {
            ...
            that.updateTips("success", "加载文件成功，文件哈希为" + hash);
            console.log('耗时：', new Date() - timeStart);
            resolve(hash);
          }
        };
